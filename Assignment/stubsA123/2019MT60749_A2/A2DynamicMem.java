...
public class A2DynamicMem extends A1DynamicMem {
    ...
    public void Defragment() {
        BSTree fragment = new BSTree();
        Dictionary i;
        for(i=freeBlk.getFirst();i!=null;i=i.getNext())
        {
            fragment.Insert(i.address,i.size,i.address);
        }
        for(i=fragment.getFirst();i!=null;i.getNext())
        {
            Dictionary temp = i.getNext();
            if(temp!=null&&i.address+i.size==temp.address)
            {
                freeBlk.Delete(temp);
                freeBlk.Delete(i);
                fragment.Delete(i);
                fragment.Delete(temp);
                freeBlk.Insert(i.address,i.size+temp.size,i.key);
                fragment.Insert(i.address,i.size+temp.size,i.key);
            }
        }
        fragment = null;
        return ;
